...
class minMaxBot(Bot):
    ...
    def miniMax(self, board, depth, player):
        ...
        if not(board.isGoal() is -1):
            return (board.heuristic(),-1)
        if depth <= 0:
            return (board.heuristic(),-1)


        children = self.generateChildren(board)

        nm = 0
        if player:
            v = -math.inf
            for c in children:
                temp = self.miniMax(c[1], depth-1, False)
                if temp[0] > v:
                    nm = c[0]
                    v = temp[0]
        else:
            v = math.inf
            for c in children:
                temp = self.miniMax(c[1], depth-1, True)
                if temp[0] < v:
                    nm = c[0]
                    v = temp[0]
        return (v,nm) # This is just an example. Replace this with your (bestScore, bestMove)
...
class alphaBetaBot(Bot):
    ...
    def alphaBeta(self, board, depth, player, alpha, beta):
        ...
        # 2) if depth is 0, return the heuristic.
        #    We don't care to return a move, so we can just return -1.
        # 3) implement AlphaBeta algorithm
        #
        # To check if the state is terminal, use isGoal() function
        # from the class Board: board.isGoal()
        #
        ...
        if player:
            v = -math.inf
            for c in children:
                temp = self.alphaBeta(c[1], depth-1, False, alpha, beta)
                if temp[0] > v:
                    nm = c[0]
                    v = temp[0]
                alpha = max(alpha, v)
            ...
            v = math.inf
            for c in children:
                temp = self.alphaBeta(c[1], depth-1, True, alpha, beta)
                vt = min(v,temp[0])
                if temp[0] < v:
                    nm = c[0]
                    v = temp[0]
